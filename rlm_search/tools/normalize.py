...
def normalize_hit(hit: dict, source_registry: dict) -> dict:
    ...
    Maps risala schema (``text`` → ``question``, ``ruling`` → ``answer``)
    so all collections produce uniform fields for format_evidence / critique.
    Also registers the result in *source_registry* (keyed by string ID).
    """
    result: dict = {
        "id": str(hit.get("id", "")),
        "score": hit.get("score", hit.get("relevance_score", 0.0)),
        "question": hit.get("question", "") or hit.get("text", ""),
        "answer": hit.get("answer", "") or hit.get("ruling", ""),
    }
    metadata: dict = {}
